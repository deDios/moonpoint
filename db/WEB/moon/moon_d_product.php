...
$sql = "DELETE FROM moon_product WHERE id = ? AND organization_id = ?";
...
if ($stmt->execute()) {
  if ($stmt->affected_rows > 0) {
    ...
    echo json_encode(["success" => false, "error" => "No se encontró el producto para eliminar"]);
  }
} else {
  if ($con->errno == 1451) {
    echo json_encode(["success" => false, "error" => "No se puede eliminar: el producto está referenciado por otros registros"]);
  } else {
