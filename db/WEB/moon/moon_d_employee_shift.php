...
if ($hard === 1) {
  $stmt = $con->prepare("DELETE FROM `moon_point`.`moon_employee_shift` WHERE organization_id=? AND id=?");
  $stmt->bind_param("ii", $organization_id, $id);
} else {
  $stmt = $con->prepare("UPDATE `moon_point`.`moon_employee_shift` SET is_active=0 WHERE organization_id=? AND id=?");
  $stmt->bind_param("ii", $organization_id, $id);
}

if (!$stmt->execute()) { echo json_encode(["success"=>false,"error"=>"Error al ejecutar: ".$stmt->error]); $stmt->close(); $con->close(); exit; }
echo json_encode(["success"=>true,"message"=> $hard===1 ? "Turno eliminado" : "Turno dado de baja"]);
