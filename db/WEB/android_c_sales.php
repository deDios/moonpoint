...
if ($sale_id > 0) {
    // === Detalle ===
    $sqlH = "SELECT id, payment_method, discount_mode, discount_value, subtotal, total, received, change_amount,
    ...
    $sqlD = "SELECT si.id, si.product_id, p.name AS product_name, si.qty, si.unit_price, si.line_total
             FROM sale_items si
...
if ($date_from && $date_to) {
    ...
    $params[] = $date_from; $types .= "ss";
    $params[] = $date_to;   $types .= "";
} elseif ($date_from) {
...
$sql = "SELECT SQL_CALC_FOUND_ROWS
            id, payment_method, subtotal, total, received, change_amount, client_name, status, created_at
        FROM sales
        $where
        ORDER BY id DESC
        LIMIT ? OFFSET ?";
...
echo json_encode(["ok"=>true, "data"=>["items"=>$list, "total"=>$totalRows, "limit"=>$limit, "offset"=>$offset]]);
mysqli_close($con);
